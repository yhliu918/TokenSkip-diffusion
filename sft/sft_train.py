...
def parse_args():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/data0/devaansh-consistency",
    ...
    parser.add_argument(
        "--consistency", type=bool, default=False, help="Use consistency loss during training"
    )
...
def load_data(args, model, tokenizer):
    ...
    train_data, eval_data = preprocess_dataset(data, model, tokenizer, args.max_length)
...
def train_model(args, tokenizer, model):
    # Load dataset
    train_dataset, eval_dataset = load_data(args, model, tokenizer)
    ...
    if args.consistency:
       trainer = dLLMConsistencyTrainer(
            model=model,
            args=training_args,
            data_collator=dLLMDataConsistencyCollator(tokenizer=tokenizer, mask_token_id=126336, max_length=args.max_length),
